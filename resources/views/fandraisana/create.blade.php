<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">🍞 Fandraisana vaovao</h2>
    </x-slot>
        ...
        <div class="bg-white shadow rounded-lg p-6 space-y-5">
            ...
            @if($kristianinas->isEmpty())
                <div class="bg-yellow-50 border border-yellow-200 text-yellow-700 px-4 py-4 rounded text-sm">
                    ✅ Kristianina rehetra vita fandraisana sahady, na tsy vita batisa. Tsy misy ilaina eto.
                </div>
            ...
            <div class="bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded text-sm">
                ℹ️ Kristianina vita batisa sady tsy mpandray ihany no hiseho eto.
            </div>

            <form action="{{ route('fandraisana.store') }}" method="POST" class="space-y-5">
                @csrf

                {{-- Kristianina --}}
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">
                        Kristianina <span class="text-red-500">*</span>
                    </label>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">
                        Daty fandraisana <span class="text-red-500">*</span>
                    </label>
                ...
                {{-- Mpanao --}}
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Mpanao fandraisana</label>
                    <input type="text" name="mpanao" value="{{ old('mpanao') }}"
